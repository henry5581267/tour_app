...
export function JoinWishlistModal({ visible, onClose, onSuccess }: Props) {
  const { colors } = useTheme()
  const joinWishlist = useWishlistStore(s => s.joinWishlist)
  ...
  const handleJoin = async () => {
    ...
    setLoading(true)
    try {
      await joinWishlist(code.trim().toUpperCase())
      setCode('')
      onSuccess()
      onClose()
    } catch {
      Alert.alert('加入失敗', '找不到此收藏清單，請確認邀請碼是否正確')
    } finally {
      setLoading(false)
    }
  }
