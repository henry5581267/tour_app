...
6. 每個地點加上 transport 欄位，代表「從前一個地點移動到此地點」建議的交通方式，只能是 "driving"（開車）、"transit"（大眾運輸）、"walking"（步行）、"bicycling"（騎車）其中之一。當天第一個地點沒有前一站，transport 設為 null。請依下列規則選擇每一段的交通方式：
   (a) 一般市區或鄰近移動：依使用者的主要交通方式偏好；距離很近時可用 walking。
   (b) 跨縣市長途移動（預估車程超過 1.5 小時，或直線距離超過 80 公里）：
       - 若使用者偏好為「自行開車」或「大眾運輸」：此段一律優先建議 transit（大眾運輸，例如高鐵或台鐵），並在「該地點的 note」加上「建議搭乘高鐵／台鐵，抵達後再轉乘」的提示。
